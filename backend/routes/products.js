...
router.get(`/`, async (req, res) =>{
    // Example: /products?categories=id1,id2&search=drill&priceMin=100&priceMax=500&ratingMin=4
    console.log(req.query)
    let filter = {};

    if (req.query.categories) {
        const categoryIds = req.query.categories.split(',').map((id) => id.trim()).filter(Boolean);
...
router.post(`/`, uploadOptions.single('image'), async (req, res) => {
    ...
    // Support multiple Cloudinary image URLs (JSON array string)
    let galleryImages = [];
    if (req.body.images) {
        try {
            galleryImages = JSON.parse(req.body.images);
        } catch (e) {
            galleryImages = [];
        }
    }
